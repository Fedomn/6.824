...
func (kv *ShardKV) getDuplicatedCommandReply(clientId, sequenceNum int64) (bool, CmdOpReply) {
	lastOperation, ok := kv.sessions[clientId]
	if ok {
		return lastOperation.SequenceNum == sequenceNum, lastOperation.Reply
	} else {
		return false, CmdOpReply{}
	}
}

// KVServer可能未收到client的RPC，而是由raft同步过来的command
func (kv *ShardKV) setSession(clientId, sequenceNum int64, reply CmdOpReply) {
...
func (kv *ShardKV) buildNotifyCh(index int) {
	kv.notifyCh[index] = make(chan CmdOpReply)
}
